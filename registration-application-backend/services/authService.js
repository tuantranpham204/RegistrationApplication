...
const { createUser, findUserByEmail, updatePassword } = require('../models/user'); // Added updatePassword
...
const login = async (email, password) => {
    ...
    const token = jwt.sign({ id: user.id, email: user.email }, process.env.SYSTEM_JWT_SECRET, {
        expiresIn: '1h'
    });
    return { token };
};
...
const forgotPassword = async(email) => {
    ...
    const token = jwt.sign({ email: user.email }, process.env.CLIENT_JWT_SECRET, {
